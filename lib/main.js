...
var renderTag = function(options, assets, attributes) {
  //set releaseVersion
  if (!options.releaseVersion) { options.releaseVersion = ''; }
  // Set attributes
  ...
  if (options.production) {
    if (options.ssl === 'relative') {
      src = '//' + options.domain + '/' + options.releaseVersion;
    } else if (options.ssl) {
      src = 'https://' + options.domain + '/' + options.releaseVersion;
    } else {
      src = 'http://' + options.domain + '/' + options.releaseVersion;
    }
    ...
    switch (method) {
      case 'uglify':
        ...
        zlib.gzip(final_code, function(err, buffer) {
          if (err) throwError(err);
          S3.putBuffer(buffer, options.releaseVersion + '/' + fileName, headers, function(err, response) {
      ...
      case 'font':
        ...
        break;
      case 'others':
        fs.readFile(assets, function(err, data) {
          if (err) throwError(err);
          zlib.gzip(data, function(err, buffer) {
            S3.putBuffer(buffer, options.releaseVersion + '/' + fileName, headers, function(err, response) {
              if (err) throwError(err);
              if (response.statusCode !== 200) {
                console.log('unsuccessful upload other type of "' + fileName + '" to S3');
                return finishUpload();
              } else {
                logger('successfully uploaded image "' + fileName + '" to S3');
                // Hack to preserve original timestamp for view helper
    ...
    if (response.statusCode === 200 && timestamp <= Date.parse(response.headers['last-modified'])) {
      ...
      switch (type) {
        ...
        default:
          ...
          async.map(assets, readUtf8, compile(fileName, assets, S3, options, 'others', type, null, finishUpload));
      }
    }
  };
};
    ...
    if (response.statusCode === 200 && timestamp <= Date.parse(response.headers['last-modified'])) {
      ...
      switch (type) {
        ...
        default:
          ...
          compile(fileName, assets, S3, options, 'others', type, timestamp, finishUpload)(null, null);
      }
    }
  };
};
  ...
  if (options.production && !options.disableWalk) {
    var walker = function() {
      var walker = walk.walk(options.viewsDir),
        results = [],
        regexCDN = /CDN\(((\([^)]+\)|[^)])+)\)/ig,
        regexAtCDN = /<%-\s+@CDN\s+(.*?)\s+%>/ig;  //support ECT templates e.g. "<%-    @CDN    '/img/apple-icon-57x57.png'    %><%- @CDN '/img/apple-icon-55x55.png' %>"
      walker.on('file', function(root, stat, next) {
        ...
        if (_.indexOf(validExts, ext) !== -1) {
          fs.readFile(path.join(root, stat.name), 'utf8', function(err, data) {
            if (err) throwError(err);
            var match;
            while ((match = regexCDN.exec(data)) !== null) {   //fix to support multiple occurance
              results.push(match[1]);
            }
              
            //add to support ECT templates
            while ((match = regexAtCDN.exec(data)) !== null) {
              results.push(match[1]);
            }
            next();
          });
